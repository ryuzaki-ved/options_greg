        ...
        <div className="max-w-5xl mx-auto">
          <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-12">
              <button className="px-6 py-3 rounded-lg bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-medium hover:from-teal-400 hover:to-cyan-400 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-teal-500/20">
                Get Started Now
              </button>
              <button className="group px-6 py-3 rounded-lg border border-white/20 bg-white/5 backdrop-blur-sm text-white font-medium hover:bg-white/10 transition-all duration-300">
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            {marketData.map((item, index) => (
              <div 
                key={index}
                className="relative p-4 bg-slate-800/40 backdrop-blur-sm rounded-xl border border-white/5 overflow-hidden group hover:bg-slate-800/60 transition-all duration-300"
              >
...
const marketData = [
  {
    ...
    value: "₹1,847 Cr",
    change: 2.8,
    chartPath: "M0,10 C10,15 20,5 30,12 S45,18 60,4 S75,8 85,15 S95,10 100,7"
  },
  {
    label: "India VIX",
    value: "14.25",
    change: -3.2,
    chartPath: "M0,7 C15,15 25,3 35,18 S50,4 65,12 S80,2 90,16 S95,8 100,14"
  },
  {
    label: "Bank Nifty PCR",
    value: "0.92",
    change: 1.5,
    chartPath: "M0,10 C15,4 25,16 40,6 S55,18 70,2 S85,14 95,8 S98,12 100,5"
  }
];
